...
* Claim: Let $$a,b \in \mathbb{Z}$$ with $$b \not = 0$$. Then, there exists $$q,r \in \mathbb{Z}$$ such that $$a = bq + r$$, where $$0 \leq r < \lvert b \rvert$$.
  ...
  Now, we need to establish that $$0 \leq r < \lvert b \rvert$$. First of all, $$r \in S \subseteq \mathbb{N}$$, thus $$r \geq 0$$ (subset of the natural numbers only contains nonnegative integers). Suppose by contradiction that $$\lvert b \rvert \leq r$$. With $$r = a - bq$$, it follows that $$\lvert b \rvert \leq a - bq$$, and we get $$0 \leq a - bq - \lvert b \rvert$$. If $$b \geq 0$$, then $$\lvert b \rvert = b$$, thus we have $$b \leq a - bq$$ and subtracting $$b$$ from the both sides of the inequality will yield $$0 \leq a - bq - b = a - b(q+1)$$. Notice that $$a - b(q+1) < a - bq$$ and $$a - b(q+1), a-bq \in S$$, meaning $$a - b(q+1)$$ should then be the minimal element of $$S$$ by the well-ordering principle. However, we already established earlier that $$a-bq$$ was the minimal element of $$S$$. You cannot have _two_ minimums in a set and have them to be different values, so this argument was contradictory to begin with, hence it must have been the case that $$r < \lvert b \rvert$$. If $$b < 0$$, then $$\lvert b \rvert = -b$$, and we will see in a similar sense that $$0 \leq a - bq + b = a - b(q - 1)$$. With $$q - 1 < q$$, we have that $$a-b(q - 1) < a-b(q)$$, meaning $$a - b(q-1)$$ indeed is the minimal element in $$S$$ again, but this is a contradiction as we established that $$a - bq$$ was the minimum. In either case, it must have been the case that $$r \leq \lvert b \rvert$$ thus the assertion for the existence is valid. $$\blacksquare$$
