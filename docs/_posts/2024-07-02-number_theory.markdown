...
* Claim: Let $$a,b \in \mathbb{Z}$$ with $$b \not = 0$$. Then, there exists $$q,r \in \mathbb{Z}$$ such that $$a = bq + r$$, where $$0 \leq r < \lvert b \rvert$$.
  ...
  1. Suppose that $$a < 0$$ and $$b < 0$$. Then, $$-b \geq 1$$ and $$-a > 0$$, thus $$(-b)(-a) = -b(-a) \geq -a$$ by multiplying the inequality $$-b \geq 1$$ by $$-a$$, thus it follows that $$a + b(a) = a - b(-a) \geq 0$$. This means we can have the same conclusion about the difference being a natural number and an element of $$S$$ like the previous case.
