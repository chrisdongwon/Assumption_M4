...
* Claim: Let $$a,b \in \mathbb{Z}$$ with $$b \not = 0$$. Then, there exists $$q,r \in \mathbb{Z}$$ such that $$a = bq + r$$, where $$0 \leq r < \lvert b \rvert$$.
  ...
  Now, we need to establish that $$0 \leq r < \lvert b \rvert$$. First of all, $$r \in S \subseteq \mathbb{N}$$, thus $$r \geq 0$$ (subset of the natural numbers only contains nonnegative integers). Suppose by contradiction that $$\lvert b \rvert \leq r$$. With $$r = a - bq$$, it follows that $$\lvert b \rvert \leq a - bq$$, and we get $$0 \leq a - bq - \lvert b \rvert$$. 