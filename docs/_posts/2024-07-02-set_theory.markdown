...
Things get extremely mind-bending when we start to consider sets with infinitely many elements. Here is something to consider: $$\lvert \mathbb{Z} \rvert = \lvert \{2k : k \in \mathbb{Z}\}\rvert$$ because the set of multiples of 2 is simply the set of integers but all elements have been multiplied by $$2$$, thus they actually should have the same number of elements. In other words, there are _just as many_ even integers as the set of integers themselves, although the set of integers have gaps between them!!!
...
## Optional: Infinity and Countability
...
So, formally, we say that a set is countable if we can list them like the natural numbers, i.e. if we can establish a _bijection_ between the set and the natural numbers. As a matter of fact, there exists a bijection from the natural numbers to the integers, and there even exists a bijection from the natural number to the rational numbers. I will refer you to this [video](https://www.youtube.com/watch?v=MqocbJ-FPo0) for a visual explanation. What about real numbers? Unfortunately, the German mathematician **Georg Cantor** established a while ago that there does NOT exist a bijection from the natural numbers to the real numbers. also known as the [diagonal argument](https://en.wikipedia.org/wiki/Cantor%27s_diagonal_argument). For this reason, the infinity involving the real numbers is _greater_ than the infinity involving the natural numbers, meaning the uncountable infinity is "greater" than the countable infinity. If you are interested, I would like you to have a glance at the notion of [transfinite ordinals](https://en.wikipedia.org/wiki/Transfinite_number), which talks about the "levels" of infinity one can discuss in mathematics. 

Lastly, if you are interested, I highly recommend that you to watch a video on [Hilbert Hotel Paradox](https://www.youtube.com/watch?v=Uj3_KqkI9Zo), which demonstrates the mind-boggling nature of the infinity of the countable type.
